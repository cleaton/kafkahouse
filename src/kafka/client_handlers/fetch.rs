...
use kafka_protocol::records::{Record, RecordBatchEncoder, RecordEncodeOptions, Compression, TimestampType};
use log::info;
...
use crate::kafka::broker::{TopicPartition, TopicPartitions};
...
pub(crate) async fn handle_fetch(
    ...
    info!("Handling Fetch request: max_bytes={}, min_bytes={}, max_wait_ms={}",
    ...
    // Fetch data from ClickHouse for all requested topics
    ...
    for topic in &request.topics {
        ...
        for partition in &topic.partitions {
            topics_map.entry(topic_name.clone())
                .or_insert_with(Vec::new)
                .push(TopicPartition::new(partition.partition, partition.fetch_offset));
        }
    }
    
    // Convert to TopicPartitions vector
    let topic_partitions: Vec<TopicPartitions> = topics_map.into_iter()
        .map(|(topic, partitions)| TopicPartitions::new(topic, partitions))
        .collect();
    ...
    if !topic_partitions.is_empty() {
        ...
        while start_time.elapsed() < max_wait {
            records = client.broker.fetch_records(&topic_partitions).await?;
            ...
            let remaining = max_wait.checked_sub(start_time.elapsed()).unwrap_or(Duration::from_millis(0));
        ...
        let mut grouped_records = std::collections::HashMap::new();
        ...
        for topic in &request.topics {
            ...
            for partition in &topic.partitions {
                ...
                let record_bytes = if !records.is_empty() {
                    ...
                    // Create records
                    let kafka_records: Vec<Record> = records.iter()
                    ...
                    RecordBatchEncoder::encode::<_, _, fn(&mut BytesMut, &mut BytesMut, Compression) -> Result<()>>(&mut buf, &kafka_records, &options)?;
    ...
    info!("Fetch response returning approximately {} records in {} bytes (waited {}ms)",
